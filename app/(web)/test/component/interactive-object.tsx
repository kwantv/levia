...
export function InteractiveObject() {
  ...
  const setResetInteraction = useStore((s) => s.setResetInteraction);
  ...
  useEffect(() => {
    ...
    const reset = (duration = 0.6) => {
      if (tiltLayerRef.current) {
        gsap.to(tiltLayerRef.current.rotation, { x: 0, y: 0, duration });
      }
      if (orbitLayerRef.current) {
        gsap.to(orbitLayerRef.current.rotation, { x: 0, y: 0, duration });
      }
      // Instant, not tweened — invisible data, only matters for what
      // useFrame reads from next time dragEnabled flips true again.
      orbit.current.x = 0;
      orbit.current.y = 0;
    };
  ...
  }, [setModelRef, setResetInteraction, orbit]);
